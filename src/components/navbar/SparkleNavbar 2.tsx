...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  const internalIndexRef = useRef(activeIndex);
  const isAnimatingRef = useRef(false);

  const createSVG = (element: HTMLDivElement) => {
    element.innerHTML = `
    <svg viewBox="0 0 116 5" preserveAspectRatio="none" class="beam">
      <path d="M0.5 2.5L113 0.534929C114.099 0.515738 115 1.40113 115 2.5C115 3.59887 114.099 4.48426 113 4.46507L0.5 2.5Z" fill="url(#gradient-beam-${color.replace('#', '')})"/>
      <defs>
        <linearGradient id="gradient-beam-${color.replace('#', '')}" x1="2" y1="2.5" x2="115" y2="2.5" gradientUnits="userSpaceOnUse">
  ...
  useLayoutEffect(() => {
    ...
    if (navRef.current && activeElementRef.current && activeButton) {
      gsap.set(activeElementRef.current, { x: getOffsetLeft(activeButton) });
      gsap.to(activeElementRef.current, { "--active-element-show": "1", duration: 0.2 });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Snap indicator when activeIndex changes externally (scroll-driven) without click animation
  useEffect(() => {
    if (isAnimatingRef.current) return;
    if (activeIndex === internalIndexRef.current) return;
    internalIndexRef.current = activeIndex;

    const newButton = buttonRefs.current[activeIndex];
    if (!activeElementRef.current || !newButton) return;
  ...
  const handleClick = (index: number) => {
    if (index === internalIndexRef.current || isAnimatingRef.current) return;

    const navElement = navRef.current;
    const activeElement = activeElementRef.current;
    const oldButton = buttonRefs.current[internalIndexRef.current];
    ...
    const direction = index > internalIndexRef.current ? "after" : "before";
    ...
    gsap.to(activeElement, {
      keyframes: [
        {
          "--active-element-width": `${Math.min(spacing, navElement.offsetWidth - 60)}px`,
          duration: 0.3,
          ease: "none",
          onStart: () => {
  ...
  return (
    <>
      <style>{`
        .sparkle-nav { position: relative; z-index: 1; }
        .sparkle-nav ul { margin: 0; padding: 0; list-style: none; display: flex; gap: 32px; }
        .sparkle-nav ul li button {
          appearance: none; border: none; cursor: pointer;
          background-color: transparent; padding: 4px 0; margin: 0;
        ...
        .sparkle-nav .active-element > svg {
          ...
          filter: blur(0.5px) drop-shadow(2px 0px 8px ${color}60) drop-shadow(0px 0px 3px ${color}80);
        }
        .sparkle-nav .active-element .strike {
          padding: 24px 0;
        ...
        .sparkle-nav .active-element .strike svg g path:nth-child(2) { filter: blur(2px); }
        .sparkle-nav .active-element .strike svg g path:nth-child(3) { filter: blur(4px); }
